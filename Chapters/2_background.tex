...
We consider the particular case in which the sets of the family are the disks, i.e. closed balls, $D_r(x)\equiv\closure{B_r}(x)= \{y\in \mathbb{R}^d \mid d(x, y) \leq r\}$ in $\mathbb{R}^d$.
...
\includegraphics[width=0.9\textwidth]{figures/bg/witnessCons.png} 
    \caption{Witness complex for a set  dataset of 11 points using a subset of 5 landmarks points and radius $r$. Source: \cite{medbouhi_towards_2022}}
...
\begin{definition}
The \emph{persistence diagram} $\text{Dgm}(f) \subset \closure{\mathbb{R}}^2$ of $f$ is the multiset of points $(a_i, a_j)$ with multiplicity $ \mu_p^{i, j}$ for all $0 \leq i < j \leq n$, union of diagonal points, $\Delta=\{(x, y) \in \closure{\mathbb{R }}^2 \mid y = x\}$, with infinite multiplicity.
\end{definition}
...
Once we have reviewed the required mathematical background, we will study some related work. First, we will focus on the relevant literature regarding \glsxtrfull{TopoML} and see how we can use the \glsxtrfull{TDA} toolkit to create a regularization technique for our \glsxtrshort{NN}. Secondly, we will focus on Representation Learning literature, where we will discuss how much we know about the latent space produced by our networks and how we can utilize that knowledge in our favor.
...
\caption{Standard TDA pipeline. Modified from: \cite{berwald_mathematics_2019}}
\label{fig:tdaPipe}
\end{figure}
...
For this purpose, we will use the conditional push-forward probability on $\mathcal{Z}$, which can be defined as $Q_k(\sigma)\equiv Q(\sigma \mid C_k) = Q(\sigma \cap C_k)/Q(C_k)$, for any class $k\in \mathcal{Y}$, event $\sigma \in \mathcal{B}$, and internal class representation $C_k=\phi(c^{-1}(k))$. Hence, we will see that if we can ensure to set an upper bound to each of the conditional probabilities of miss-classifying the decision region of that given class, then we will reduce the generalization error. This statement is formalized in the following lemma:

\begin{lemma}[\cite{hofer_densified_2021}]
\label{lem:genErr}
For any class $k\in \mathcal{Y} = [K]$, let $C_k=\phi(c^{-1}(k))$ be its internal representation, and $D_k= \gamma^{-1}(k)$ be its decision region in $\mathcal{Z}$ w.r.t $\gamma$. If exists $\epsilon >0$ s.t.
\begin{equation}
...
\[
M_{l\cdot \epsilon} = \bigcup_{z\in M} \closure{B_{l\cdot \epsilon}}(z)\,.
\]
...
\subsection{Representation Comparison \& Model Stitching \todo{I don't know if this title is a good fit}}
\label{sec:repLearn}

\textbf{intro:} say that we still dont know much about the latent space of the network. State that representation learning tries to either study how to "automatically" obtain good representations of our dataset (instead of tailored features such as SWIFT in CV). Also in this field and in general in ML there is an interest to understand more about about our latent spaces, and a way to asses some properties of the latent space will requiere us to be able to compare one to another.\\

Therefore, we are in the setup where we want to compare the layer $L_1$ of network $f_\theta$ with the layer $L_2$ of the network $g_\phi$. Notice that the dimensionality of the two networks can be completely different. For simplicity, we will suppose w.l.o.g that we can vectorize the output of both layers into $\mathbb{R}^{d_1}$ and $\mathbb{R}^{d_2}$, respectively, and that $d_1 \leq d_2$.
...
\mathbf{h}_i, \mathbf{x}_j \rangle}
    ...
    \begin{equation}
    \label{eq:CKA}
    \text{CKA}(K, L) = \frac{\text{HSIC}(K,L)}{\sqrt{\text{HSIC}(K,L) \cdot \text{HSIC}(K,L)}}\,.    
    \end{equation}
